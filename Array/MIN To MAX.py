...
Output Format:
For each test case, output on a new line, the minimum number of operations required to make M the maximum value
in the array A.
...
Since all elements of the final array are 1, the maximum value of the array is now 1. It can be shown that this
is the minimum number of operations required to do so.

Test case 2: The minimum value in the array, MM, is initially 2. We can use two operations as following:
...
Test case 3: The minimum value in the array, M, is initially 1. It is also the maximum value of the array.
Hence, no operations are required.
"""
...
while t > 0:
    ...
    # Your code goes here
    min_value = min(a)
    count_min_value = a.count(min_value)
    
    print(n - count_min_value)
    t = t-1
